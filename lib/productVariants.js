import { productPathPrefixFor } from "@/lib/categories";
...
export async function hydrateVariantSiblings(Product, products) {
  ...
  const siblings = await Product.find({ $or: queries })
    .select('slug title brand volume price compareAtPrice stock images concentration gender')
    .lean();
...
export function productPath(product, byId) {
  if (!product) return '/perfumes';
  const slug = product.slug || product._id;
  if (!slug) return '/perfumes';
  return `${productPathPrefixFor(product, byId)}/${slug}`;
}

export function canonicalVariant(variants = [], fallback = null) {
  const list = (variants || []).filter(Boolean);
  if (!list.length) return fallback;
  return [...list].sort(sortByVolume)[0];
}
